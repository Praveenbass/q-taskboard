...
from .airtable_exporter import export_tasks
    ...
    def get(self, request, project_id):
        ...
        if q:
            with connection.cursor() as cursor:
                sql = (
                    f"SELECT id, project_id, title, description, status, "
                    f"assignee_id, created_by_id, position, created_at, updated_at "
                    f"FROM tasks "
                    f"WHERE project_id = '{project_id}' "
                    f"AND (title ILIKE '%{q}%' "
                    f"OR description ILIKE '%{q}%') "
...
class ExportView(APIView):
    def post(self, request, project_id):
        ...
        # Fetch every task belonging to this project.
        tasks = (
            Task.objects
            .filter(project_id=project_id)
        ...
        try:
            result = export_tasks(task_data)
        except Exception:
            return Response(
                {'error': 'Airtable export failed'},
